...
// A group of instanced cubes: http://metalbyexample.com/instanced-rendering/
class CubeSetPrimitive : Primitive {
    // static properties are evaluated lazily :) device should be ready!
    private static let indexBuffer : MTLBuffer! = CubeSetPrimitive.createCubeIndexBuffer()
    private static let vertexBuffer : MTLBuffer! = CubeSetPrimitive.createCubeVertexBuffer()
    ...
    var instanceTransforms : [Transform]
    ...
    static func createCubeIndexBuffer() -> MTLBuffer {
        let buffer = RenderManager.sharedInstance.createIndexBuffer("cube IB", elements: CubeSetPrimitive.triangleList)
        return buffer
    }
    ...
    override func draw(encoder: MTLRenderCommandEncoder) {
        encoder.setVertexBuffer(CubeSetPrimitive.vertexBuffer, offset: 0, atIndex: 0)
        ...
        encoder.drawIndexedPrimitives(.Triangle, indexCount: CubeSetPrimitive.triangleList.count, indexType: .UInt16, indexBuffer: CubeSetPrimitive.indexBuffer, indexBufferOffset: 0, instanceCount: instanceTransforms.count)
    }
