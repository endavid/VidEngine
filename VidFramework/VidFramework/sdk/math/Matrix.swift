...
public extension float4x4 {
    ...
    {
        let m = float4x4([
            simd_float4(2 * near / (right - left), 0, 0, 0),
            simd_float4(0, 2 * near / (top - bottom), 0, 0),
            simd_float4((right + left) / (right - left), (top + bottom) / (top - bottom), -(far + near) / (far - near), -1),
            simd_float4(0, 0, -2 * far * near / (far - near), 0)]
        )
        return m
    }
    ...
    static func createFrustumInverse(left: Float,right: Float,bottom: Float,top: Float,near: Float,far: Float) -> float4x4
    {
        ...
        let m = float4x4([
            simd_float4((right - left) * invNear, 0, 0, 0),
            simd_float4(0, (top - bottom) * invNear, 0, 0),
            simd_float4(0, 0, 0, (near - far) * invNearFar),
            simd_float4((right + left) * invNear, (top + bottom) * invNear, -1, (far + near) * invNearFar)]
        )
        return m
    }
    ...
    init(rowMajorElements v: [Float]) {
        let col0 = simd_float4(v[0], v[4], v[8], v[12])
        let col1 = simd_float4(v[1], v[5], v[9], v[13])
        let col2 = simd_float4(v[2], v[6], v[10], v[14])
        let col3 = simd_float4(v[3], v[7], v[11], v[15])
