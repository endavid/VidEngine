...
public class CubePrimitive : Primitive {
    fileprivate static var cubeIB: MTLBuffer?
    fileprivate static var cubeVB: MTLBuffer?
    ...
    public init(renderer: Renderer, instanceCount: Int) {
        ...
        if CubePrimitive.cubeIB == nil {
            CubePrimitive.cubeIB = CubePrimitive.createCubeIndexBuffer(renderer)
        }
    ...
    static func createCubeIndexBuffer(_ renderer: Renderer) -> MTLBuffer {
        let buffer = renderer.createIndexBuffer("cube IB", elements: CubePrimitive.triangleList)
        return buffer
    }
    
    static func createCubeVertexBuffer(_ renderer: Renderer) -> MTLBuffer {
        let uv0 = Vec2(0, 0)
        let buffer = renderer.createTexturedVertexBuffer("cube VB", numElements: 6 * 4)
